...
**If A:** Construct the plan review prompt. Read the plan file being reviewed. If a CEO plan document
...
finding as a decision brief in chat and getting explicit approval. This applies even when you
...
Follow the Decision Brief Format from the skill above. Additional rules for plan reviews:
...
### TODOS.md updates
Present each potential TODO as its own individual decision brief. Never batch TODOs — one per brief. Never silently skip this step.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
If any decision brief goes unanswered, note it here. Never silently default.

## Review Log

After producing the Completion Summary above, write the review result to the plan file.
...
Produce this markdown section and write it to the END of the plan file as the terminal heading:
...
- **CROSS-MODEL:** (only if outside voice ran and found tension) — summary of cross-model agreement/disagreement
...
2. If found, use the Edit tool to DELETE the entire existing section. Match from
   ...
   file, whichever comes first. Replace with the empty string.
3. After the delete (or skipped, if no section existed), append the new
   `## PLAN REVIEW REPORT` section at the END of the file.
...
**Recommend /plan-eng if eng review has not run** — eng review is the required shipping gate. If this CEO review expanded scope, changed architectural direction, or accepted scope expansions, emphasize that a fresh eng review is needed.
...
Present a decision brief to present the next step. Include only applicable options:
...
At the end of the review, if the vision produced a compelling feature direction, offer to promote the CEO plan to the project repo. Present a decision brief:
...
- **B)** Keep in the kg plan folder only (local, personal reference)
- **C)** Skip

If promoted, copy the CEO plan content to `docs/designs/{FEATURE}.md` (create the directory if needed).
