...
In plan mode, allowed because they inform the plan: Read, Grep, Glob, Bash (read-only), writes to the plan file, and `open` for generated artifacts.
...
**A) <option label>** ✅ pick · <human ~Xm / AI ~Ym>
- ✅ <pro, one line>
- ✅ <pro, one line>
- ❌ <con, one line>
...
- **Options:** one bold header line per option; mark the recommended one inline with `✅ pick`; put effort inline as `human ~Xm / AI ~Ym` on effort-bearing options. Then ≥2 ✅ pros and ≥1 ❌ con as one-line bullets. Hard-stop escape for one-way/destructive confirmations: a single `- ✅ No cons — hard-stop choice`.
...
- [ ] Effort inline (human / AI) on effort-bearing options
...
Skills that run plan reviews include the EXIT PLAN MODE GATE blocking checklist at the end of the skill, which verifies the plan file ends with `## PLAN REVIEW REPORT` before ExitPlanMode is called. Writing the plan file is the one edit allowed in plan mode.

## Plan Selection

Follow the **Plan Selection Pattern** (see `commands/plan/README.md`, Storage + Plan Selection Pattern sections).

Short form for this skill (status filter: `todo` first, then `doing`):

1. If the user passed an explicit plan path, use it directly.
2. Run `~/.claude/scripts/kg-plan.sh resolve`. On success: `REPO`, `SLUG`, `TODO_DIR`, `DOING_DIR` are known. Look under `plans/{repo}/todo/{slug}/` first (pre-implementation review); if absent, try `plans/{repo}/doing/{slug}/`.
   - Found: auto-select, announce `Selected: {path}`.
   - Not found in either: fall through to step 3.
   - Non-zero exit (not a ticket branch, detached HEAD, no origin): fall through to step 3.
3. Fallback: `~/.claude/scripts/kg-plan.sh list {repo} todo` (or `doing`). 0 found → error. 1 found → auto-select. Multiple → show selector.
4. If `kg-plan.sh` is not installed or exits non-zero with no usable output: fall back gracefully to the plan currently in context (plan mode) or ask the user for a path. Never hard-fail.

Reference `commands/plan/README.md` (Storage + Plan Selection Pattern) for full detail.
...
Print the detected base branch name. In every subsequent `git diff`, `git log`, and git command, substitute the detected branch name wherever the instructions say "the base branch" or `<default>`.
...
|------|-------------------|
| running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed) | `~/.claude/skills/plan-design/sections/review-sections.md` |
---
...
### 0D. Focus Areas
Present a decision brief: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll review all 7 dimensions. Want me to focus on specific areas instead of all 7?"

**STOP.** Do NOT proceed until user responds.

## Design Outside Voices (parallel)

Present a decision brief in chat:
...
Present subagent output under a `INDEPENDENT DESIGN REVIEW:` header.

**Pass integration:**
- Critical findings → raised as the FIRST items in Pass 1
- Other findings → pre-loaded as known issues in the relevant pass
...
5. Present a decision brief if there's a genuine design choice to resolve
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.claude/skills/plan-design/sections/review-sections.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Section self-check (before you finish)

Confirm you Read the review section the Section index named, and executed all 7 design passes, the required outputs, and the review report in full. If you produced findings or the review report from memory without Reading `~/.claude/skills/plan-design/sections/review-sections.md`, stop and Read it now.
...
2. Confirm the LAST `## ` heading in the file is `## PLAN REVIEW REPORT`.
   In-body prose that mentions "outside voice" or similar does NOT count — only
   the structured `## PLAN REVIEW REPORT` section satisfies this check.
3. Confirm the report contains: a Runs / Status / Findings table, a VERDICT
   line, and absorbs UNRESOLVED lines if applicable.
4. If a plan file is in context for this skill invocation: confirm the plan file
   was updated and the review report section is present at the end. If no plan
   file is in context, checks 1-3 already short-circuit.
